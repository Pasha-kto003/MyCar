...
namespace MyCar.Server.DataModels
{
    public static class ModelData
    {
        ...
        public static CarApi GetCar(Car car, MyCar_DBContext dbContext)
        {
            ...
            foreach (var ch in characteristicCars)
            {
                ch.Characteristic = (CharacteristicApi)dbContext.Characteristics.FirstOrDefault(s => s.Id == ch.CharacteristicId);
                ch.Characteristic.Unit = (UnitApi)dbContext.Units.FirstOrDefault(s=> s.Id == ch.Characteristic.UnitId);
            }
            ...
            result.Model = (ModelApi)model;
            result.Model.MarkCar = (MarkCarApi)dbContext.MarkCars.FirstOrDefault(s => s.Id == result.Model.MarkId);
        ...
        public static OrderApi OrderGet(Order orderIn, MyCar_DBContext dbContext)
        {
            ...
            var user = dbContext.Users.FirstOrDefault(x => x.Id == orderIn.UserId);
            result.User = (UserApi)user;
            result.User.UserType = (UserTypeApi)dbContext.UserTypes.FirstOrDefault(s => s.Id == result.User.UserTypeId);
            result.User.Passport = (PassportApi)dbContext.Passports.FirstOrDefault(s=> s.Id == result.User.PassportId);
